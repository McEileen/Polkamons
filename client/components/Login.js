...
export default class Polkamon extends React.Component {
  ...
  create(e) {
    ...
    console.log(body);
    fetch('//localhost:3333/api/login', { method: 'post', body, headers: { 'Content-Type': 'application/json' } })
    .then((r) => localStorage.addItem({ token: r }))
    .then(() => browserHistory.push('/polkamon'));
  }
